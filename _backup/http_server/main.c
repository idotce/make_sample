...
extern const char *getdeviceattr();
extern const char *capability();
extern const char *getproductinfo();
extern const char *getmediainfo();

static int s_debug_level = MG_LL_INFO;
static const char *s_root_dir = ".";
static const char *s_listening_addr = "http://0.0.0.0:80";
...
static void cb(struct mg_connection *c, int ev, void *ev_data) {
  ...
  if (ev == MG_EV_HTTP_MSG) {
    ...
    else if (mg_match(hm->uri, mg_str("/app/getdeviceattr"), NULL)) {
      const char *json_response = getdeviceattr();
      mg_http_reply(c, 200,
                  "Content-Type: application/json\r\n",
                  "%s", json_response);
    }
    else if (mg_match(hm->uri, mg_str("/app/capability"), NULL)) {
      const char *json_response = capability();
      mg_http_reply(c, 200,
                  "Content-Type: application/json\r\n",
                  "%s", json_response);
    }
    else if (mg_match(hm->uri, mg_str("/app/getproductinfo"), NULL)) {
      const char *json_response = getproductinfo();
      mg_http_reply(c, 200,
                  "Content-Type: application/json\r\n",
                  "%s", json_response);
    }
    else if (mg_match(hm->uri, mg_str("/app/getmediainfo"), NULL)) {
      const char *json_response = getmediainfo();
      mg_http_reply(c, 200,
                  "Content-Type: application/json\r\n",
                  "%s", json_response);
    }
    ...
    MG_INFO(("%.*s %.*s %.*s -> %.*s",
            ...
            3, c->send.buf + 9));
    if (hm->body.len > 0) {
      MG_INFO(("%.*s", hm->body.len, hm->body.buf));
    }
  }
}
...
int main(int argc, char *argv[]) {
  ...
  MG_INFO(("Version          : v%s", MG_VERSION));
