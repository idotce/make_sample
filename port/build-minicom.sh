...
export CFLAGS="-Os -static -I${TOOLCHAIN}/aarch64-linux-gnu/include -I${pwd_dir}/_install/userdata/include"
export LDFLAGS="-static -L${TOOLCHAIN}/aarch64-linux-gnu/lib -L${pwd_dir}/_install/userdata/lib -lpthread -lncursesw"

# 工程代码
base_dir=$pwd_dir/_download
pack_url=https://mirror.truenetwork.ru/slackware/slackware/source/a/minicom/minicom-2.8.tar.lz
pack_file=$pwd_dir/_download/minicom-2.8.tar.lz
proj_dir=$pwd_dir/_download/minicom-2.8
build_dir=$pwd_dir/_download/minicom-2.8/_build
...
if [ ! -d "$proj_dir" ]; then
    lzip -dc "$pack_file" | tar -xvf - -C "$base_dir" || { echo "解压源码失败!"; exit 1; }
fi
